...
export const VotingProvider = ({ children }) => {
  ...
  const getAllCandidatedata = async () => {
    try {
      ...
      allCandidateData.map(async (ele) => {
        ...
        setCandidateIndex((prevState) => [
          ...prevState,
          singleCandidateData[2].toNumber(),
        ]);
  ...
  const ApprovedCandidate = async (candidateAddress, message) => {
    try {
      ...
      const transaction = await contract.approveCandidate(
        candidateAddress,
        message
      );
      await transaction.wait();
  ...
  const RejectCandidate = async (candidateAddress, message) => {
    try {
      ...
      const transaction = await contract.rejectCandidate(
        candidateAddress,
        message
      );
      await transaction.wait();
  ...
  const uploadToIPFS = async (file) => {
    ...
    const JWT =
      "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..9beZK2zfysefAZBKJFUqjB1AxRPAh87eNHJHDjwsGfc";
  ...
  const setelectionTime = async (startTime, endTime) => {
    try {
      ...
      setCurrentVotingStatus("Set successfully");
      //Redirect
      // window.location.href = "/";
    } catch (error) {
  ...
  const getCurrentVotingStatus = async (currentTime) => {
    try {
      const web3Modal = new Web3Modal();
      const connection = await web3Modal.connect();
      const provider = new ethers.providers.Web3Provider(connection);
      ...
      const votingStatus = await contract.currentVotingStatus(currentTime);
  ...
  const fetchWinner = async (currentTime) => {
    try {
      const web3Modal = new Web3Modal();
      const connection = await web3Modal.connect();
      const provider = new ethers.providers.Web3Provider(connection);
      ...
      const [winningAddress, name, voteCount] =
        await contract.getWinningCandidate(currentTime);
